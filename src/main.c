...
int main(int argc, char **argv) {
	...
	bcm2835_gpio_fsel(SWITCH_PIN, BCM2835_GPIO_FSEL_INPT);
	bcm2835_gpio_set(SWITCH_PIN);
	bcm2835_gpio_set_pud(SWITCH_PIN, BCM2835_GPIO_PUD_UP);	  
		
	while(1)
	{
	    data = bcm2835_gpio_lev(SWITCH_PIN);	
		if (data == 0)
...
int main(void) {
  ...
  if ( wiringPiISR (SWITCH_PIN, INT_EDGE_FALLING, &button_pressed_callback) < 0 ) {
